...
function zywhm() {
    ...
    items.push({
        ...
        url: $('hiker://empty#x#fypage@-1@#x#').rule(() => {
            ...
            var lin = 'https://api.cupfox.app/api/v2/search/?text=' + getMyVar('xyqzywcjtext') + '&type=0&from=' + spl[1] * 10 + '&size=20&token='+tok;
            var lint = 'https://api.cupfox.app/api/v2/search/?text=' + getMyVar('xyqzywcjtext') + '&type=0&from=' + spl[1] * 24 + '&size=24&token='+tok;
            var pn = spl[1] * 1 + 1;
            try{
    ...
    items.unshift({
        title: '香情影视',
        url: 'hiker://home@香情影视||https://code.aliyun.com/lzk23559/Rulehouse/raw/master/香情影视口令.txt',
        col_type: 'flex_button'
    });
    items.unshift({
        title: 'APP影视',
        url: 'hiker://home@APP影视(P)||https://code.aliyun.com/lzk23559/Rulehouse/raw/master/APP影视口令.txt',
        col_type: 'flex_button'
    });
    items.unshift({
...
function danmufun(src) {
    ...
    try {
        ...
        //parwix解析弹幕接口
        var json = JSON.parse(request("https://jx.bozrc.com:4433/dmku/?ac=dm&id=" + md5(src.split('"')[0].split('?')[0]).slice(12)+ ' P', {}));
        if (json.danmuku) {
...
//x5解析的线路
}else if(flag == 'zbkplayer'||flag == 'ltnb'||flag == 'dym3'||flag == 'tpiframe'||flag=='Tcm3u8'|| flag == 'renrenmi'||flag=='VIP'||flag=='xyb'||flag=='bst'||flag=='weibo'){
return x5rule(src);
